...
char *gremlin_log_file = NULL;
char *gremlin_replay_file = NULL;
bool gremlin_replay_gdb = false;
char *murphy_config_file = NULL;
...
#endif  // Mangle for murphy

	printf("Use: %s [options] <command> ...\n",cmd);
	printf("Where options and environment variables are:\n");
	printf("  -c <file>  Path to Murphy config file.         (MURPHY_CONFIG)\n");
	printf("  -g <file>  Send gremlin activation log to this file. \n");
	printf("  -r <file>  Replay a gremlin activation log from this file. \n");
	printf("  -a         Attach w/ gdb when replay of a gremlin activation log completes. \n");
	printf("  -d <name>  Enable debugging for this sub-system.  \n");
	printf("  -H         Disable use of helper library.\n");
	printf("  -h         Show this screen.\n");
	printf("  -l <path>  Path to ld.so to use.                      \n");
	printf("  -o <file>  Send debugging messages to this file.     \n");
	printf("  -O <bytes> Rotate debug files of this size.     \n");
	printf("\n");
	printf("Known debugging sub-systems are: ");
	// debug_flags_print(stdout);

	printf(" syscall notice process pstree alloc cache poll debug murphy user all time pid \n");
...
int main( int argc, char *argv[] )
{
	...
	while((c=getopt(argc,argv,"+hA:ab:B:c:Cd:DE:FfG:Hi:kKl:m:M:N:o:g:r:O:p:QR:sSt:T:U:u:vw:WYZ"))!=(char)-1) {
		switch(c) {
		case 'g':
			gremlin_log_file = optarg;
			break;
		case 'r':
			gremlin_replay_file = optarg;
			break;
		case 'a':
			gremlin_replay_gdb = true;
			break;
		case 'b':
